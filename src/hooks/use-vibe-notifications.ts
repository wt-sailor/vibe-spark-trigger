...
export function useVibeNotifications() {
  ...
  const [messages, setMessages] = useState<Array<{ type: string; payload: unknown; time: string }>>([]);
  ...
  const register = useCallback(async (externalUserId: string) => {
    ...
    try {
      ...
      vibeClient.onMessage((payload: unknown) => {
        setMessages(prev => [
          { type: 'foreground', payload, time: new Date().toLocaleTimeString() },
          ...prev,
        ]);
        const p = payload as { title?: string; body?: string };
        const message = [p?.title || 'New notification', p?.body].filter(Boolean).join('\n');
        toast.info(message);
      });

      vibeClient.onBackgroundMessage((payload: unknown) => {
        setMessages(prev => [
          { type: 'background', payload, time: new Date().toLocaleTimeString() },
          ...prev,
        ]);
      });
      ...
      vibeClient.onSilentMessage((data: unknown) => {
        setMessages(prev => [
          { type: 'silent', payload: data, time: new Date().toLocaleTimeString() },
          ...prev,
        ]);
      });
  ...
  return { isRegistered, userId, isRegistering, messages, register, unregister };
}
